...
def get_dataset_class(name):
    if name == "humanml": # TODO:delete once done - used as a reference
        ...
        return FAMOS
    elif name in ["express4d", "Express4D_reaction_nofilter_v1"]:
        return Express4D
    else:
...
def get_dataset(name, num_frames, split='train', hml_mode='generator', data_mode='landmarks', normalize_data=False, classifier_step=15, minimum_frames = 60, debug=False, smoothing_filter_length = 7, add_velocities=False, add_landmarks_diffs = False, max_len=196, flip_face_on=False, fps=20, max_motions=-1):
    ...
    if name in ["famos", "voca", "coma"]:
        dataset = DATA(name=name, split=split, num_frames=num_frames, mode=hml_mode, data_mode=data_mode, normalize_data=normalize_data,classifier_step=classifier_step, minimum_frames=minimum_frames, debug=debug, smoothing_filter_length= smoothing_filter_length, add_velocities=add_velocities, add_landmarks_diffs=add_landmarks_diffs, max_len=max_len)
                       # datapath=f'dataset/FAMOS_data_train_test_split.json')
    elif name in ["express4d", "Express4D_reaction_nofilter_v1"]:
        dataset = DATA(name=name, split=split, mode=hml_mode, data_mode=data_mode, minimum_frames=minimum_frames, debug=debug, max_len=max_len, flip_face_on = flip_face_on, fps=fps, max_motions=max_motions)
    # elif name in ["coma"]:
    #     dataset = DATA(name=name, split=split, num_frames=num_frames, mode=hml_mode, data_mode=data_mode, datapath='dataset/COMA_data')
    else:
        dataset = DATA(name=name, split=split, num_frames=num_frames)
    return dataset
